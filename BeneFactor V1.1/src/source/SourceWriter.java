...
public class SourceWriter {
	...
	SourceWriter(IMethod methodToAppendTo) throws JavaModelException {
		...
		String lineSeparator = Platform.getPreferencesService().getString(
				...
				System.getProperty("line.separator"), scopeContext);
		...
		int lastBodyLineEndingIndex = source.lastIndexOf(lineSeparator,
				closingBracketIndex);
		if (lastBodyLineEndingIndex > 0) {
			prefix = source.substring(0, lastBodyLineEndingIndex
					+ lineSeparator.length())
					+ lineSeparator;
			suffix = lineSeparator
					+ source.substring(lastBodyLineEndingIndex
							+ lineSeparator.length());
		} else {
	...
	/**
	 * Writes out a line to the source writer using the current indentation
	 * level. Empty lines are not indented.
	 * 
	 * @param line
	 ...
	 * {@link IMethod} to append to, then you must use the buffer of that
	 * method, and only the method will be replaced.
	 * 
	 * @param buffer
	...
	public void commit(IBuffer buffer) throws JavaModelException {
		if (methodRange != null) {
			buffer.replace(methodRange.getOffset(), methodRange.getLength(),
					toString());
		} else {
	...
	public String toString() {
		...
		TextEdit textEdit = codeFormatter.format(formatKind, source, 0,
				source.length(), indentationLevel, null);
