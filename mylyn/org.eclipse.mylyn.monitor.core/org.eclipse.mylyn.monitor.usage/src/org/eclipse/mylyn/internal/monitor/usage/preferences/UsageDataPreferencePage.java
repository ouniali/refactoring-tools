...
public class UsageDataPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private void createUsageSection(Composite parent) {
		...
		//TODO added a new button; need to implement on_click;
		new Label(group, SWT.NONE).setText("");
		Button upload_now = new Button(group, SWT.NONE);
		upload_now.setText("Upload Now");
		upload_now.addSelectionListener(new SelectionListener() {
			public void widgetDefaultSelected(SelectionEvent arg0) {
			}

			public void widgetSelected(SelectionEvent arg0) {
				UiUsageMonitorPlugin.getDefault().uploadData(true);
			}
		});
	}
