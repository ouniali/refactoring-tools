...
public class UiUsageMonitorPlugin extends AbstractUIPlugin {
	...
	public static final long DEFAULT_DELAY_DAYS_BETWEEN_TRANSMITS = 1;
	...
	private CheckForUploadJob checkForUploadJob;
	...
	public void uploadData(boolean instant) {

		checkForUploadJob = new CheckForUploadJob(PlatformUI.getWorkbench().getDisplay());
		final boolean original = checkForUploadJob.getInstantUpload();
		checkForUploadJob.setInstantUpload(instant);
		checkForUploadJob.setSystem(true);
		checkForUploadJob.schedule(0);//TODO was START_CHECK_UPLOAD_JOB_DELAY instead of 0
		checkForUploadJob.addJobChangeListener(new JobChangeAdapter() {
			@Override
			public void done(IJobChangeEvent event) {
				checkForUploadJob.setInstantUpload(original);
				if (event.getResult() != null && event.getResult().isOK()) {
					checkForUploadJob.schedule(CHECK_UPLOAD_JOB_INTERVAL);// TODO was CHECK_UPLOAD_JOB_INTERVAL
				}
			}
		});
	}
	...
	private void initializeDefaultStudyParameters() {
		...
		//http://mylyn.eclipse.org/monitor/upload/MylarUsageUploadServlet
		//http://localhost:8080/webpagepractice/UploadMylynDataServlet
		//TODO
		studyParameters.setUploadServletUrl("http://152.14.92.76:8080/webpagepractice/UploadMylynDataServlet"); //$NON-NLS-1$
