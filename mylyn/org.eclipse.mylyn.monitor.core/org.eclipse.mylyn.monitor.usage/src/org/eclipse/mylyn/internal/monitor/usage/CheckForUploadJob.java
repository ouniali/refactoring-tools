...
public class CheckForUploadJob extends UIJob {
	...
	public void setInstantUpload(boolean in) {
		instant_upload = in;
	}
	...
	synchronized void checkForStatisticsUpload() {
		...
		//TODO change the code to instantly submit without worrying about day
		if (shouldAskForUpload(currentTime) || instant_upload) {
			...
			MessageDialog message = new MessageDialog(Display.getDefault().getActiveShell(),
					...
					new String[] { Messages.UiUsageMonitorPlugin_Submit_Feedback,
					//NLS.bind(Messages.UiUsageMonitorPlugin_Remind_Me_In_X_Days, getUserPromptDelay(), ending),
					//Messages.UiUsageMonitorPlugin_Dont_Ask_Again, //TODO was a button for not asking again.
					}, 0);
